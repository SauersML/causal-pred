...
\title{Causal Disease Modeling with Structural and Parametric Uncertainty; Type-2 Diabetes}
...
\begin{abstract}
Disease risk prediction from biobank data is often accomplished with supervised learning. On standard observational data, this process fails to capture ignores causal structure and lacks explicit uncertainty about which variables drive disease causation. Causal DAG inference techniques, on the other hand, typically do not generate individual-level predictions. We combine Mendelian-randomisation (MR) with Bayesian structure learning to produce individualised T2D risk with causal estimates, while propagating parametric (i.e., coefficient-level) and structural uncertainty (i.e., model-level). MR estimates are converted into an edge-inclusion prior by a Wakefield-style spike-and-slab Bayes factor applied to MR effects. This is residualised against the indirect-path contributions implied by the graph. A DAGSLAM-like method gives us a warm-start DAG over mixed continuous, binary, and right-censored survival nodes. These are scored with BGe and Laplace-approximated logistic marginal likelihoods. To enable uncertainty about the graphs themselves, structure MCMC is used to sample from the posterior over graphs. These posterior parent sets of the T2D node are passed to a survival generalized additive model which explicitly models how noise varies across the input variables. Individual survival curves are Bayesian model-averaged over parent node sets. On a synthetic biobank cohort of 5{,}000 subjects across 18 mixed-type nodes, this method recovers 17 of 20 ground-truth causal edges at posterior probability $\ge 0.5$, corresponding to an edge AUROC of 0.945 and an AUPRC of 0.611 against a permutation-null AUPRC of 0.072 ($p<0.002$). Calibration is good (calibration error 0.081) and the integrated Brier score is 0.122, better than a Kaplan--Meier baseline of 0.151. On the All of Us biobank, the strongest inferred edge was HbA1c, a blood sugar indicator very relevant to T2D physiology.
\end{abstract}
...
\end{tikzpicture}
\caption{MR assumptions: the variant affects the outcome through the exposure, and that nothing causes both the variant and the outcome (dashed gray arrows ruled out). When these assumptions are met, the outcome association represents a causal effect of the exposure on the outcome.}
\label{fig:mr-dag}
\end{figure}

Bayesian structure learning over DAGs scores each candidate graph by a marginal likelihood with a conjugate structure prior \citep{Geiger2002, Kuipers2014} and explores the space via Markov chain Monte Carlo with neighborhood-balanced proposals \citep{GiudiciCastelo2003, MadiganYork1995}. Bayesian model averaging over the posterior propagates structural uncertainty into downstream predictions \citep{Draper1995}.
...
\subsection{Data exploration on All of Us}
\label{sec:data-exploration}

To explore the All of Us biobank data, we assessed $102{,}052$ participants with genome and electronic health records. $2{,}423$ had type~2 diabetes. For each participant, we created $117$ polygenic-score features from the PGS Catalog, and $941$ EHR-derived features across $72$ condition codes, $834$ drug-exposure codes, and $35$ lab results. We then trained a TopK cross-modal sparse autoencoder over the PRS+EHR input space ($d=2048$ latent dimension size, $k=32$ active per row) to understand the data organization and determine if T2D is trivially clustered with some combination of input features.

Figure~\ref{fig:participant-umap} shows the resulting participant latent space projected to two dimensions via UMAP. Type~2 diabetes are not uniform across embedding, indicating that the variation captured by the principal PRS and EHR axes is not completely captured by any feature group. We also recover multiple biological axes: a metabolic-syndrome axis (top loadings on BMI and metabolic-syndrome PGSs together with HbA1c and triglyceride lab summaries), a hypertension and cardiovascular axis (hypertrophic-cardiomyopathy and venous-thrombosis PGSs with anti-hypertensive drug exposures), and a lipid axis (LDL- and HDL-cholesterol PGSs with statin and lipid-modifying drug exposures). This indicates that DAG construction is feasible, and motivates the node set used in the rest of the paper.



\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{figures/crosscoder_10_participant_umap.png}
\caption{Participant latent space on All of Us (UMAP projection of TopK cross-modal sparse-autoencoder activations, $n=12{,}000$ subsampled from $102{,}052$). Light grey points are non-T2D participants ($n=9{,}577$ in the subsample); dark points are T2D cases ($n=2{,}423$). (Section~\ref{sec:data-exploration}).}
\label{fig:participant-umap}
\end{figure}
...
\]
on a requested time grid. \texttt{gamfit} is queried for survival probabilities at these horizons. The GAM structurally enforces monotonicity of the survival curve over time, since survival probabilities should not increase as the time horizon grows.

We obtain response-scale standard errors for the predicted survival probabilities using \texttt{gamfit}'s survival uncertainty interface. These standard errors represent uncertainty within a single fitted survival model. This uncertainty term includes the uncertainty of the spline coefficients themselves in addition to sampling error.
...
\bottomrule
\end{tabular}
\caption{End-to-end benchmark table. Higher is better for Nagelkerke $R^2$, time-dependent AUC, and edge AUPRC; lower is better for the integrated Brier score (IBS).}
...
\caption{Held-out synthetic benchmark.  Top-left: integrated time-dependent AUC over $[5,15]$ y.  Top-right: Nagelkerke $R^2$ at 10 y.  Bottom-left: scaled integrated Brier score (IBS / IBS$_{\mathrm{KM}}$; the dotted line marks the Kaplan--Meier reference at 1.0, lower is better).  Bottom-right: causal-edge discovery AUROC for MR-IVW vs.\ causal-pred.  causal-pred uses the gamfit survival-GAMLSS backend averaged over the parent-set posterior; baselines (Cox PH, naive logistic, Kaplan--Meier) use all 17 covariates directly.  Generated by \texttt{scripts/benchmark.py}.}
\label{fig:benchmarks}
\end{figure}

\paragraph{Benchmark comparison.}
We compared the full pipeline against Kaplan--Meier and Cox proportional hazards baselines on various metrics, using two GAM backends. On the gamfit backend, Causal-Pred attains the best Nagelkerke $R^2$ at 10 years (0.245 vs.\ 0.186 logistic, 0.167 Cox), the best ten-year time-dependent AUC (0.828 vs.\ 0.745, 0.741), and the lowest integrated Brier score among survival models (0.127 vs.\ 0.132, 0.141), with an edge AUPRC of 0.546 against 0.530 for MR--IVW. On the R mgcv/gamlss backend, which does not include posterior sampling, performance is lower: Cox PH is the best (Nagelkerke 0.312, td-AUC 0.807, IBS 0.121), and Causal-Pred edge recovery is AUPRC 0.586 and AUROC 0.809. This may indicate that better uncertainty allows also for increased discrimination due to the maximum a posteriori estimates becoming more accurate.

\paragraph{Limitations of biobank data.}
Though a synthetic cohort with a known ground-truth DAG is useful since the causal structure in the real world is not known, All of Us individual-level data are accessible inside the Researcher Workbench and were also used \citep{AllofUs2024}. On real data, HbA1c was discovered as a strong risk factor. This is also used to diagnose type 2 diabetes, which complicates the interpretation, but also validates that the model can recover relevant nodes.

\section{Reproducibility}

All code is pinned via \texttt{uv.lock}. The GAM backend is the Rust-based \texttt{SauersML/gam} package built through \texttt{rustup}. Polygenic scoring uses the \texttt{gnomon} CLI. RNGs are used. A full run is reproduced by \texttt{uv run python scripts/run\_full\_pipeline.py}.
