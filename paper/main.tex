...
In addition to parent-set resampling, the sampler optionally supports another Gibbs-style kernel. The edge-pair Gibbs move chooses an unordered node pair $\{i,j\}$ and samples among the legal states “no edge,” $i \to j$, and $j \to i$ from their conditional posterior. This move is optional, but it provides an additional way to move through graph space when ordinary single-edge proposals mix slowly.
