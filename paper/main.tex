...
The \texttt{gamfit} Python library was created to implement this survival model in Python, as existing Python GAM libraries do not have support for survival modeling or serious uncertainty quantification. The fitted model is a right-censored location--scale GAMLSS with a Gompertz--Makeham baseline cumulative hazard,
\[
H_0(t) \;=\; \alpha\,t \;+\; \frac{\lambda}{\gamma}\!\left(e^{\gamma t}-1\right),
\]
whose free parameters $\alpha,\lambda,\gamma > 0$ are estimated jointly with the smooth terms. Each of the $m$ parent covariates in $x$ contributes two penalized cubic B-spline smooths -- one to a location predictor $\eta_\mu$ and one to a scale predictor $\eta_\sigma$,
\[
\eta_\mu(x) \;=\; \sum_{j=1}^{m} f_{j}^{(\mu)}(x_j),
\qquad
\eta_\sigma(x) \;=\; \sum_{j=1}^{m} f_{j}^{(\sigma)}(x_j),
\]
each built on a $k=5$ cubic B-spline basis with a second-difference penalty whose smoothing parameter is selected by REML. The conditional survival function composes the baseline cumulative hazard with a location--scale shift,
\[
S(t \mid x) \;=\; \bar{F}\!\left(\,H_0(t) - \eta_\mu(x)\,e^{-\eta_\sigma(x)}\,\right),
\]
where $\bar{F}=1-F$ is the survival function of a fixed parametric residual distribution and start--stop data \texttt{Surv(entry, exit, event)} contribute their left-truncation correction through $H_0(\texttt{entry})$.
