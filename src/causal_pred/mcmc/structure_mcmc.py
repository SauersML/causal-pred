...
def _gibbs_resample_parents(
    ...
    # top-12 predecessors by MrDAG prior log-odds collapses this to
    # ~sum_k C(12, k) ~= 1.6k -- fewer than cap=3 unpruned -- while
    # preserving the 4th/5th-parent ambiguity that drives BMA structural
    # uncertainty. Pruned candidates can still enter the chain via the
    # single-edge MH moves, so the chain remains ergodic over the full
    # DAG space; only the *Gibbs conditional* is restricted, not the
    # marginal posterior the chain targets.
    if len(candidates) > _GIBBS_TOP_K_CANDIDATES:
        cand_scores = [(float(log_pi[i, j] - log_1m[i, j]), int(i)) for i in candidates]
        cand_scores.sort(reverse=True)
        candidates = [i for _, i in cand_scores[:_GIBBS_TOP_K_CANDIDATES]]
...
def _run_chain(
    ...
    for it in range(total_iters):
        ...
        # Exact parent-set Gibbs move with probability ``hybrid_prob``.
        if hybrid_prob > 0.0 and kernel_u < edge_resample_prob + hybrid_prob:
...
def run_structure_mcmc(
    ...
    **hyper,
    ...
    max_parents : int
        Maximum number of incoming edges per node.  Both single-edge
        moves and exact parent-set resampling stay within this bounded
        DAG space.
    n_samples : int
    ...
    hybrid_prob : float, default 0.1
        Per-iteration probability of proposing an exact parent-set
        Gibbs update (Madigan & York 1995 Section 4) instead of a
        single-edge move.  ``0.0`` disables the move.
    ...
    # ``max_parents = None`` means "no cap"; substitute the natural upper
    # bound p so every code path can treat it as a finite integer.
